...
public class ItemsC implements Initializable {
	    ...
	    try {
	        ...
	        if (connection != null) {
	            ...
	            while (results.next()) {
	                ...
	                String itemPicture = results.getString("picture");
	                items.add(new Item(iid, name, price, cid, itemPicture));
	            }
	...
	public void updateCategories() {
		try {
			FXMLLoader fxmlloader = new FXMLLoader();
			fxmlloader.setLocation(getClass().getResource("/application/layout/UpdateCatsPU.fxml"));
			DialogPane popup = fxmlloader.load();
			
			UpdateCatsPU uCats = fxmlloader.getController();
			...
			if(clickedBtn.get() == ButtonType.OK) {
				try {
					connection = MysqlConnection.getDBConnection();
					if(uCats.catid == 0) {
						sql = "INSERT INTO `categories` (`name`, `picture`) VALUES(?, ?)"; 
						ps = connection.prepareStatement(sql);
						
						ps.setString(1, uCats.getName());
						ps.setString(2, uCats.savePic());
					} else {
